 ...
 */
  ...
  public ContinousThroughputTest() {
    sendThread = new Thread("sendThread") {public void run()  {parser(); } };
    receiveThread = new Thread("receiveThread") {public void run()  {checkChannel(); } };
...
//    Starts input Parser
  ...
  public void checkChannel() {
    ...
    Message msg=null;
    long start=0,stop=0,totalLatency=0,max=0,min=1000,latency=0;
    long troughput;
    float meanLatency;
    boolean done=false,burstReceiving=false;
    int receivedMsgs=0,pings=0;
    long n;
    int i=1;

    System.out.println("Started receiving");
    try {
      byte[] buf = Util.objectToByteBuffer("ciao");
      while (!done) {
	...
	if (received instanceof Message) {
	  ...
	  if (payload.equalsIgnoreCase("stop")) { done = true; }
...
//	    i = ((Integer) ((Message) channel.receive(0)).getObject()).intValue();
	...
	try {logWriter.write("Stopped cause " + ex + "\r\n");} catch (Exception e) {}
	System.exit(2);
    }
  ...
  public void parser() {
    ...
    try {
      while (!done) {
	    ...
	    if (input.equalsIgnoreCase("stop")) {done = true; }
