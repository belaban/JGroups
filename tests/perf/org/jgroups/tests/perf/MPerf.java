...
public class MPerf implements Receiver {
    ...
    protected static void printOutput(String s) {
    ...
    protected void sendNoException(Address target, Object payload, byte header, Message.Flag ... flags) {
        Message msg=new Message(target, payload).setFlag(flags);
        if(header > 0)
    ...
    protected class Sender implements Runnable {
        ...
        public void run() {
            ...
            while(running.get()) {
                try {
                    Message msg=new Message(null, payload).putHeader(ID, new MPerfHeader(MPerfHeader.DATA));
                    if(oob)
    ...
    protected static class Result implements Streamable {
        ...
        public void writeTo(DataOutput out) throws IOException {
            ...
            Bits.writeInt(sources_length, out);
            if (sources_length == 0) {
                return;
            }
            for (int i = 0; i < sources.length; i++) {
                Util.writeAddress(sources[i], out);
                Bits.writeLong(received[i], out);
            }
        }
        ...
        public void readFrom(DataInput in) throws IOException, ClassNotFoundException {
            ...
            int sources_length = Bits.readInt(in);
            if (sources_length > 0) {
