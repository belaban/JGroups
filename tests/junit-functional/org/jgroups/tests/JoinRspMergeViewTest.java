...
public class JoinRspMergeViewTest {
    ...
    public void testJoinRspFromUnpatchedCoord() throws Exception {
        View view=new MergeView(new ViewId(a, 5), Arrays.asList(a, b, c),
                                Arrays.asList(View.create(a, 1, a, b), View.create(c, 1, c)));
        Buffer buf=marshalLegacy(view, createDigest(view));
        try {
            JoinRsp rsp=Util.streamableFromBuffer(JoinRsp::new, buf.getBuf(), buf.getOffset(), buf.getLength());
    ...
    protected static Buffer marshalLegacy(View view, Digest digest) throws Exception {
    ...
    protected static void _testRoundTrip(View view) throws Exception {
        ...
        Buffer buf=Util.streamableToBuffer(new JoinRsp(view, digest));
        JoinRsp rsp=Util.streamableFromBuffer(JoinRsp::new, buf.getBuf(), buf.getOffset(), buf.getLength());
