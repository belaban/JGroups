...
The second method accepts a list of membership lists; each list represents a subview that needs to get
            ...
            adds it to the resulting list and the adds the subviews in turn. This could result in a MergeView like
            +{M,A,B,C,N,O,P,X,Y,Z}+.
...
NOTE: In both regular and merge views, it is important that there are no duplicate members. It is entirely
      ...
      result in C or D being present in the resulting merged view multiple times.
...
The FORK protocol of the main stack contains 2 fork stacks: "counter" and "lock". There are
...
The design of FORK / ForkChannel is discussed in more detail at
