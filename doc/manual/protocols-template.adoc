...
|id | Gives the protocol a different ID if needed so we can have multiple instances of it in
      the same stack
|after_creation_hook | Fully qualified name of a class implementing ProtocolHook, will be called after creation of
      the protocol (before init())
| preview_warning | If true, log a warning if a protocol annotated with @Preview is used
| level | Sets the logging level. Valid values are "fatal", "error", "warn", "info", "debug", "trace"
...
use-localhost:: Returns the result of calling `InetAddress.getLocalHost()`.
...
Both `TCP` and `TCP_NIO2` connect to other members via TCP/IP. If a TCP/IP connection is closed by the peer, we don't know
whether the peer left gracefully (ie., disconnected its channel) or crashed (and the operating system closed the socket).

To differentiate between crash-leave and graceful leave, a `GRACEFUL_CLOSE (-5)` is sent to the peer when disconnecting
a channel gracefully, just before closing the socket. Therefore, when a member receives a `-5` from peer P, it knows
that P left gracefully when the connection is subsequently closed and doesn't emit a `SUSPECT(P)` event.

All other closes assume that P crashed and therefore a `SUSPECT(P)` event is sent up the stack.

NOTE: The `GRACEFUL_CLOSE (-5)` is sent as *length* (but without payload), so the network wire format is not changed
and is therefore backwards compatible.

To enable this feature, set `enable_suspect_events` to `true`.
...
As of 5.5.3, `NAKACK4` can be configured (`dynamic_buffers`) to use dynamic (unbounded) retransmission buffers; this
prevents blocking, but it is recommended to use a multicast flow control (such as MFC) to avoid out-of-memory failures.
...
NOTE: Non-blocking flow control protocols were removed in 5.5.0: https://issues.redhat.com/browse/JGRP-2925.
...
side, the smaller fragments will get assembled into larger messages again, and delivered to the application.

FRAG and FRAG2 work for both unicast and multicast messages.
            
The difference between FRAG and FRAG2 is that FRAG2 does one less copy than FRAG, so it is the recommended
...
FRAG3 needs only half the memory of FRAG2 to handle fragments and the final full message. When the first fragment is
received, a message with the full buffer already allocated is created. When fragments are received, they're copied into
the full message and immediately discarded. When complete, the full message is passed up.

This reduces memory allocation caused by keeping fragments in memory until all of them have been received.

For details see https://issues.redhat.com/browse/JGRP-2154 for details.


==== FRAG4
FRAG4 is able to handle ObjectMessages in addition to BytesMessages.
...
STATS exposes various statistics, e.g. number of received multicast and unicast messages, number of bytes sent etc.
It should be placed directly over the transport
...
[[SIZE]]
==== SIZE
`SIZE` records all received and sent messages per *message length*. It is typically placed towards the top
of the stack, but can be placed lower, e.g. if internal messages need to be analyzed.

A typical output looks like this (edited):
----
[mac] /Users/bela/JGroups$ probe SIZE.dumpUpSamples[]
#1 (423 bytes):
mac-54345(v=16.0.0) [ip=192.168.1.110:7803, 4 mbr(s), cluster=default, version=5.4.9.Final-SNAPSHOT (Alpe d'Huez) (java 24+36-3646)]
SIZE.dumpUpSamples=

...

#4 (424 bytes):
mac-33618(v=16.0.0) [ip=192.168.1.110:7800, 4 mbr(s), cluster=default, version=5.4.9.Final-SNAPSHOT (Alpe d'Huez) (java 24+36-3646)]
SIZE.dumpUpSamples=
1: 1’110’328
29: 284’516
30: 550’647
...
Most received messages (`2’795’048`) had a length (`Message.getLength()`) of `1'026` bytes.

${SIZE}

==== SIZE2
[[SIZE2]]
Contrary to `SIZE`, `SIZE2` doesn't record every different size, but sizes are placed into _buckets_, e.g. from
0-100, 101-1000, 1001-1500, 1501-2000, 2001-100000 etc. This can be configured as follows

[source.xm]
----
 <SIZE2 sizes="0,100,1000,1500,2000,100000" />
----
This configures 4 buckets and all messages are recorded in one of them. A possible output might look like this (edited):
----
...
We can see that `mac-24327` recorded `4’778’180` received messages of lengths between `1001-1500` bytes.
