...
However, I still needed to compile and install the Ensemble
runtime for each different platform, which is exactly why Java was
developed in the first place: portability.
...
working on a pure Java implementation of the group communication transport (now: ProtocolStack).
...
That's why, after my time was up, I left Cornell and
academia altogether, and started a job in the telecom industry in Silicon Valley.


At around that time (May 2000), SourceForge had just opened
...
I want to thank all contributors to JGroups, present and past, for their work. Without you,
...
include::./writing.adoc[Creating a new protocol]
