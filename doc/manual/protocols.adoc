...
The `dns_address` parameter is optional and when it's missing, the protocol will use the default DNS server configured
on the machine (usually, it's the first entry of `/etc/resolv.conf`).
