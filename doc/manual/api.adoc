...
Oftentimes different configurations have parts in common which change, but the rest is the same. An example
for the latter is transports such as `TCP` or `UDP`. These could be defined once, and be included by various
...
NOTE: An include can configure *multiple protocols*: for example, we could define both a transport *and* a
discovery protocol
