...
This is done by creating a new `BytesMessage` (a message containing the destination and sender's addresses,
plus a byte array as payload) and calling `Channel.send()` with it as argument.

The first argument of the BytesMessage constructor is the destination address. A null destination address
...
serialize the object ourselves (which is actually recommended!) and use the BytesMessage contructor which
