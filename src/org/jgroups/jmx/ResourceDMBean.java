...
public class ResourceDMBean implements DynamicMBean {
        ...
        try {           
            ...
            if(mbean.description() != null && mbean.description().trim().length() > 0){
                if (log.isInfoEnabled()) {
                    log.info("@MBean description set - " + mbean.description());
                }
                this.description = mbean.description();
                vctAttributes.add(new MBeanAttributeInfo(ResourceDMBean.MBEAN_DESCRITION, "java.lang.String", "@MBean description", true, false, false));
            }else{
                this.description = "";
            }            
            	...
            	for (Field field : fields) {
                    ...
                    if (attr != null) {
                        if (Modifier.isFinal(field.getModifiers())) {
                            ...
                            vctAttributes.add(new MBeanAttributeInfo(field.getName(), field.getType().getCanonicalName(), attr.description(), attr.readable(), false, false));
                        } else {
                            vctAttributes.add(new MBeanAttributeInfo(field.getName(), field.getType().getCanonicalName(), attr.description(), attr.readable(), attr.writable(), false));
                        }
                        if (log.isInfoEnabled()) {
            ...
            for (Method method : methods) {
                ...
                if (attr != null) {
                    if (method.getParameterTypes().length == 0 && method.getReturnType() != java.lang.Void.TYPE) {
                        vctAttributes.add(new MBeanAttributeInfo(method.getName() + "()", method.getReturnType().getCanonicalName(), attr.description(), attr.readable(), false, false));
                        if (log.isInfoEnabled()) {
    ...
    public synchronized void setAttribute(Attribute attribute) {
        if (log.isDebugEnabled()) {
            log.debug("setAttribute called for " + attribute.getName() + " value " + attribute.getValue());
        }
    ...
    public synchronized AttributeList setAttributes(AttributeList list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (setNamedAttribute(attr)) {
                ...
                if (log.isWarnEnabled()) {
                    log.debug("Failed to update attribute name " + attr.getName() + " with value " + attr.getValue());
                }
            }
        }
        return results;
