// $Id: MessageDispatcher.java,v 1.12 2004/04/21 13:33:03 yaron-r Exp $
...
public class MessageDispatcher implements RequestHandler {
	 ...
	 * has completed before fetching the next item from the queue. Note that setting this to true may destroy the
	 * properties of a protocol stack, e.g total or causal order may not be guaranteed. Set this to true only if you know
	 * what you're doing !
	 */
	...
	/**
	 * If this dispatcher is using a user-provided PullPushAdapter, then need to set the members from the adapter initially
	 * since viewChange has most likely already been called in PullPushAdapter.
	 */
	...
	public void start() {
		if (corr == null) {
			...
			corr.start();
			//We have a correlator, we can 'up'. Unlock the up lock.
			prot_adapter.releaseUpLock(); //calling release on an already 'released' lock has no side effects.
		}
	...
	public void stop() {
		//We're stopped. Lock the up lock to make sure we don't loose events. They will be queued by the up queue of Protocol.
		prot_adapter.acquireUpLock(); //release it in start()
		if (corr != null) {
			corr.stop();
			corr = null;
		}
	 ...
	 * @param mode    Defined in <code>GroupRequest</code>. The number of responses to wait for: <ol> <li>GET_FIRST: return
	 *                the first response received. <li>GET_ALL: wait for all responses (minus the ones from suspected
	 *                members) <li>GET_MAJORITY: wait for a majority of all responses (relative to the grp size)
	 *                <li>GET_ABS_MAJORITY: wait for majority (absolute, computed once) <li>GET_N: wait for n responses
	 *                (may block if n > group size) <li>GET_NONE: wait for no responses, return immediately (non-blocking)
	 *                </ol>
	...
	public RspList castMessage(final Vector dests, Message msg, int mode, long timeout) {
		...
		if (log.isDebugEnabled()) {
			log.debug("real_dests=" + real_dests);
		}
		if (real_dests == null || real_dests.size() == 0) {
			{
				if (log.isInfoEnabled()) {
					log.info("destination list is empty, won't send message");
				}
	 ...
	 * @param coll   The sender needs to provide this interface to collect responses. Call will return immediately if this
	 *               is null
	 */
	...
	/**
	 * Sends a message to a single member (destination = msg.dest) and returns the response. The message's destination must
	 * be non-zero !
	 */
	public Object sendMessage(Message msg, int mode, long timeout) throws TimeoutException, SuspectedException {
		...
		if (rsp_list.size() > 1) {
			if (log.isWarnEnabled()) {
				log.warn("response list contains " +
...
//    }
	...
	class ProtocolAdapter extends Protocol implements UpHandler {
		...
		/**
		 * Called by request correlator when message was not generated by it. We handle it and call the message listener's
...
//                    break;
		...
		public void up(Event evt) {
			//todo: Yaronr: queue up events when correlator is null.

/* Not needed any more. stop() blocks the upping, start() restarts it.
...
*/
			if (corr != null) {
				...
				/* This is not an error, we are between stop and start
				if (log.isErrorEnabled()) {
					log.error("corr == null");
				} */
			}
		...
		/**
		 * A situation may exist where an up protocol exists, but it is not ready to process events yet. In that case we
		 * should save the events until he is ready. should this queue be bounded - yes, to be on the safe side. 50 should be
		 * more than enough, oldest event should be pushed out.
		 * <p/>
		 * This method is used by Protocol to test if the dispatcher is ready to process events.
		 */
