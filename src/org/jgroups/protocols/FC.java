 ...
 * doc/FlowControl.txt
 * @author Bela Ban
 * @version $Id: FC.java,v 1.68 2007/03/30 14:50:27 belaban Exp $
 */
        ...
        try {
            if(lowest_credit <= length) {
                ...
                while(insufficient_credit && running) {
                    try {mutex.await(max_block_time, TimeUnit.MILLISECONDS);}
                    ...
                    if(insufficient_credit && running) {
                        ...
                        List<Address> creditors_copy=new ArrayList(creditors);
                        lock.unlock();
                        try {
                            if(trace)
