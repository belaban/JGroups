...
public void setUp(long mesNums, int mesSize, int num_threads) throws Exception {
    final Sender[]      senders=new Sender[num_threads];
    final CyclicBarrier barrier=new CyclicBarrier(num_threads +1);
    final byte[]        payload=new byte[mesSize];

    for(int i=0; i < num_threads; i++) {
        //senders[i]=new Sender(barrier, seqno, payload, mesNums, mesSize, this);
        senders[i].setName("sender-" + i);
        senders[i].createChannel();
    }
//    try {
...
//    }
    ...
    channel1=new JChannel("conf/sequencer.xml");
    if(this.name != null)
        channel1.name(name);
    //channel.setReceiver(this);
    channel1.connect("ChatCluster");
    local_addr=channel1.getAddress();
    JmxConfigurator.registerChannel(channel1, Util.getMBeanServer(), "jgroups", "ChatCluster", true);

    Address coord=channel1.getView().getMembers().get(0);
    setUp(30000, 1000, 5);
}
...
//
//    for(int i=0; i < num_threads; i++) {
//    	MyReceiver receiver = new MyReceiver(this, name);
//        senders[i]=new Sender(barrier, seqno, payload, mesNums, mesSize,  this);
//        senders[i].setName("sender-" + i);
//        senders[i].start();
//    }
        ...
        try {
            ...
            switch(c) {
                case '1':
                    ...
                    break;
                case '2':
                    System.out.println("view: " + channel1.getView() + " (local address=" + channel1.getAddress() + ")");
                    break;
                case '3':
                ...
                case '5':
                	...
                	break;
                case '6':
                    ProtocolStack stack=channel1.getProtocolStack();
                    String cluster_name=channel1.getClusterName();
                    try {
                        JmxConfigurator.unregisterChannel(channel1, Util.getMBeanServer(), "jgroups", "ChatCluster");
                    }
                    catch(Exception e) {
                    }
...
public class Sender extends Thread {
    ...
    private  AtomicLong seq = new AtomicLong(0);//, actually_sent;
...
//        if(this.name != null)
//            channel.name(name);
        channel.setReceiver(new MyReceiver(zt));
    ...
    public void run() {
        ...
        for  (int i = 0; i < numsMsg; i++) {
            ...
            try {
   	    	    MessageId messageId = new MessageId(local_addr, seq.getAndIncrement()); // Increment localSequence
...
private static class MyReceiver extends ReceiverAdapter {
    ...
    public void receive(Message msg) {
        ...
        if (testHeader != null && testHeader.getType()==ZABHeader.START_SENDING){
        	...
        	msgReceived=0;
        	try {
				//zabTest.sendMessages(30000, 1000,5);
			} catch (Exception e) {
...
//    	    System.out.println("message received = zxid  is = " + testHeader.getZxid()+" "+ msgReceived + " at "+ zabTest.getCurrentTimeStamp());
//    	    System.out.println("Throughput = " + (end - zabTest.getStartTime())/1000000);
    	    //System.out.println("Test Done ");
        }
    }
}
