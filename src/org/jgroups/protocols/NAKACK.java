// $Id: NAKACK.java,v 1.1.1.1 2003/09/09 01:24:10 belaban Exp $
...
public class NAKACK extends Protocol {
     ...
     */
     void rebroadcastMsgs(Vector v) {
		...
		h1=(NakAckHeader)m1.getHeader(getName());
		for(int j=0; j < final_v.size(); j++) {
		    m2=(Message)final_v.elementAt(j);
		    h2=(NakAckHeader)m2.getHeader(getName());
	...
	private class LastMessageRetransmitter implements TimeScheduler.Task {
	    ...
	    public long    nextInterval()  {return retransmit_timeout[0];}
	...
	void stable(long[] seqnos) {
	    ...
	    seqno=seqnos[index];
	    if(Trace.trace) Trace.info("NAKACK.stable()", "deleting stable messages [" +
				       deleted_up_to + " - " + seqno + "]");
	...
	void receiveAck(long id, Address sender) {
	    if(Trace.trace) Trace.info("NAKACK.NAKer.receiveAck()",
	...
	*/
	...
	public void retransmit(long first_seqno, long last_seqno, Address sender) {
	    if(Trace.trace) Trace.info("NAKACK.NAKer.retransmit()",
