...
public class FD_SOCK2 extends Protocol implements Receiver, ConnectionListener, ProcessingQueue.Handler<FD_SOCK2.Request> {
    ...
    protected NioServer createServer(int[] bind_ports) {
        ...
        if (threadFactory instanceof DefaultThreadFactory dft && dft.useVirtualThreads() && Runtime.version().feature() < 25) {
            // With JDK < 25, the EPollSelectorImpl.doSelect keeps the virtual thread busy,
            // preventing the carrier thread to be used by other virtual threads
            // It is a problem with systems with a low CPU count.
            // It can be removed after JGroups adopts JDK 25 as minimum version.
            threadFactory = new LazyThreadFactory("jgroups", true, true)
