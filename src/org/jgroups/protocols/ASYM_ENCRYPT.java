...
public class ASYM_ENCRYPT extends EncryptBase {
    ...
    @Override protected Object handleUpEvent(Message msg, EncryptHeader hdr) {
        switch(hdr.type()) {
            ...
            case EncryptHeader.SECRET_KEY_RSP:
                ...
                break;
            default:
                log.warn(String.format("%s: received unknown encrypt header of type %d", local_addr, hdr.type()));
                break;
        }
        return null;
    }
    ...
    protected void handleSecretKeyRequest(final Message msg) {
        if(!inView(msg.src(), "%s: key requester %s is not in current view %s; ignoring key request"))
            return;
    ...
    protected void handleSecretKeyResponse(final Message msg, final byte[] key_version) {
        if(!inView(msg.src(), "%s: ignoring secret key sent by %s which is not in current view %s"))
            return;
        try {
    ...
    protected void drainUpQueue() {
        ...
        while((queued_msg=up_queue.poll()) != null) {
            try {
                ...
                log.error(String.format("failed decrypting message from %s: %s", queued_msg.src(), ex));
            }
        }
    }
