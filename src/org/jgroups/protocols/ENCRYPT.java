...
//	 for client to store server's public Key
	...
	private void initSymCiphers(String algorithm, SecretKey secret) throws Exception
	{
		...
		symVersion = new String(digest.digest());
		if (log.isInfoEnabled())
	...
	private void becomeKeyServer(Address tmpKeyServer){
		...
		keyServer =true;
		if (log.isInfoEnabled())
			log.info("I have become key server");
		queue_down = false;
		queue_up = false;
	}
	...
	private void handleUpMessage(Event evt) throws Exception
	{
		...
		{
			if (log.isDebugEnabled())
				log.info("Dropping message as ENCRYPT header is null  or has not been recognized, msg will not be passed up");
			return;
		}
	...
	private  void drainUpQueue() throws QueueClosedException, Exception
	{
		synchronized(upLock){
			while (up_queue.size() != 0){
				Event tmp = (Event)up_queue.remove();
	...
	public void down(Event evt)
	{
		...
		switch (evt.getType()) {
			...
			case Event.MSG :
				if (evt.getArg() == null || ((Message)evt.getArg()).getBuffer() == null){
					log.debug("passing down message as is empty message or buffer ");
					break;
				} 
				try
				{
	...
	private void drainDownQueue() throws Exception, QueueClosedException
	{
		synchronized(downLock){
			...
			while(down_queue.size() != 0){
				sendDown((Event)down_queue.remove());
			}
		}
	}
	...
	private void sendDown(Event evt) throws Exception
	{
		...
		msg.putHeader(EncryptHeader.KEY, new EncryptHeader(
				EncryptHeader.ENCRYPT, getSymVersion()));

		if (msg.getBuffer() != null)
		{
			encryptMessage(symEncodingCipher, msg);

		} else
		{
			if (log.isInfoEnabled())
				log.info("buffer is null not encrypting ");
		}
		passDown(evt);
	}
	...
	private Message encryptMessage(Cipher cipher, Message msg) throws Exception
	{
		...
		// message in readable form
		if (log.isDebugEnabled())
			log.debug(" Encypher message:" + formatArray(msg.getBuffer()));
