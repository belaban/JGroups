 ...
 * @author Bela Ban
 * @version $Id: SFC.java,v 1.10.2.7 2007/04/18 06:35:54 belaban Exp $
 */
public class SFC extends Protocol {
    ...
    public boolean setProperties(Properties props) {
        ...
        Util.checkBufferSize("unicast_max_credits", unicast_max_credits);
    ...
    class FlowCoordinator {
        ...
        boolean handleDownMessage(Event evt, Message msg) {
            ...
            try {
                ...
                while(curr_credits_available <= 0 && running) {
                    ...
                    if(ignore_synchronous_response && ignore_thread == Thread.currentThread()) {
                        if(trace)
                            log.trace(targetLabel + "Bypassing blocking to avoid deadlocking " + Thread.currentThread());
                    ...
                    if(trace)
                        log.trace(targetLabel + "blocking (current credits=" + curr_credits_available + ")");
                    try {
                        ...
                        if(curr_credits_available <= 0 && running) {
                            if(trace)
                                log.trace(targetLabel + "returned from timedwait but curr_credits_available=" + curr_credits_available);
        ...
        void handleUpMessage(Message msg, Address sender) {
            ...
            if(credit_response > 0)
                sendCreditResponse(sender, credit_response, multicast);
        }
        ...
        private void sendCreditResponse(Address dest, long credit, boolean multicast) {
...
//             credit_rsp.setFlag(Message.OOB);
    ...
    public static class Header extends org.jgroups.Header implements Streamable {
        ...
        public String toString() {
            switch(type) {
                case MULTICAST_REPLENISH:
                    return "MULTICAST_REPLENISH";
                case MULTICAST_CREDIT_REQUEST:
                    return "MULTICAST_CREDIT_REQUEST";
                case UNICAST_REPLENISH:
                    return "UNICAST_REPLENISH";
                case UNICAST_CREDIT_REQUEST:
                    return "UNICAST_CREDIT_REQUEST";
                default:
                    return "<invalid type>";
            }
        }
