...
public abstract class EncryptBase extends Protocol {
    ...
    protected Object handleUpMessage(Message msg) throws Exception {
        ...
        if(hdr == null) {
            log.error(String.format("%s: received message without encrypt header from %s; dropping it", local_addr, msg.src()));
            return null;
        }
        switch(hdr.type()) {
    ...
    protected Object handleEncryptedMessage(Message msg) throws Exception {
        ...
        log.warn(String.format("%s: unrecognized cipher; discarding message from %s", local_addr, msg.src()));
        return null;
    }
    ...
    protected Message decryptMessage(Cipher cipher, Message msg) throws Exception {
        ...
        if(!Arrays.equals(hdr.version(), sym_version)) {
            ...
            log.trace(String.format("%s: decrypting msg from %s using previous cipher version", local_addr, msg.src()));
    ...
    protected Message _decrypt(final Cipher cipher, Message msg, EncryptHeader hdr) throws Exception {
        ...
        if(encrypt_entire_message && sign_msgs) {
            ...
            if(signature == null) {
                log.error(String.format("%s: dropped message from %s as the header did not have a checksum", local_addr, msg.src()));
                return null;
            }
